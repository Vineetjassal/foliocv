...
function Editor() {
  ...
  useEffect(() => {
    ...
    // If data exists and savedAt exists, it came from localStorage (persisted draft)
    if (data && savedAt) {
      // Only show toast when navigation source is NOT from /create (i.e. a direct refresh)
  ...
  // Visual save-status debounce (the real persist is instant via zustand middleware)
  useEffect(() => {
    if (!data) return;
